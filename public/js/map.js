...
export async function loadAndDisplayAds(filters = {}) {
    try {
        ...
        // Délégation d'événement pour les boutons dans les popups
        map.on('popupopen', function (e) {
            const btn = e.popup._container.querySelector('[data-action="open-ad-detail"]');
            if (btn) {
                btn.addEventListener('click', () => {
                    const adId = btn.dataset.id;
                    // Il faudra créer cette fonction dans ui.js pour ouvrir la modale de détail
...
function geolocateUser() {
    if ('geolocation' in navigator) {
        navigator.geolocation.getCurrentPosition(position => {
            ...
            const userIcon = L.divIcon({ className: 'pulsing-marker-visuals', iconSize: [24, 24] });
